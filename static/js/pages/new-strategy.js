...
const NewStrategyPage = {
    ...
    container.innerHTML = `
      <div class="page-enter" style="max-width:640px;margin:0 auto;">
        <div style="margin-bottom:28px;">
          ...
          <p style="color:var(--text-muted);font-size:14px;">Paste a YouTube URL, webpage, keywords, or the strategy's own source (e.g. a Pine Script)</p>
        </div>
        ...
        <div class="card" style="padding:28px;">
          ...
          <div style="display:flex;gap:6px;background:rgba(255,255,255,.04);padding:4px;border-radius:10px;margin-bottom:24px;">
            ${[
              ...
              { id: 'text', label: 'Paste Code/Text', icon: '<polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/>' },
            ].map(t => `
  ...
  _renderInput() {
    ...
    if (this._tab === 'text') {
      return `<textarea class="input-base" id="source-input" rows="12" style="font-family:var(--font-mono, monospace);font-size:12.5px;" placeholder="Paste a Pine Script strategy, a written rule set, or any other strategy source text..."></textarea>`;
    }
  ...
  async _submit() {
    ...
    const isTextInput = this._tab === 'keyword' || this._tab === 'text';
    if (!val) { Toast.warning('Please enter ' + (isTextInput ? 'some text' : 'a URL')); return; }
    ...
    try {
      ...
      if (isTextInput) payload.keywords = val;
